...
if [ ${comm_type} == 'eth' ]; then
    ...
    cmd="/usr/local/src/pysmurf-custom-transmitter-example/server_scripts/cmb_eth.py  ${args}"
else
    echo "Staring the server using PCIe communication..."
    cmd="/usr/local/src/pysmurf-custom-transmitter-example/server_scripts/cmb_pcie.py ${args}"
fi
